...
class CheckpointMergerPipeline(DiffusionPipeline):
    ...
    def _compare_model_configs(self, dict0, dict1):
        if dict0 == dict1:
            ...
            if config0 == config1:
                print(
                    f"Warning !: Mismatch in keys {meta_keys0} and {meta_keys1}.")
                return True
        return False
            ...
            if not attr.startswith("_"):
                ...
                if os.path.exists(checkpoint_path_1):
                    files = list(
                        (
                            *glob.glob(
                                os.path.join(checkpoint_path_1,
                                             "*.safetensors")
                            ),
                    ...
                    if os.path.exists(checkpoint_path_2):
                        files = list(
                            (
                                *glob.glob(
                                    os.path.join(
                                        checkpoint_path_2, "*.safetensors")
                                ),
                                *glob.glob(os.path.join(checkpoint_path_2, "*.bin")),
                            )
                        )
                        checkpoint_path_2 = files[0] if len(
                            files) > 0 else None
                ...
                try:
                    ...
                    print(
                        f"Skipping {attr} do to an unexpected error: {str(e)}")
                    continue
