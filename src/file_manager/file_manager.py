...
class FileManager:
    ...
    def init_app(self, app):
        ...
        app.config.setdefault("THUMBNAIL_MEDIA_ROOT",
    ...
    def get_thumbnail(
        ...
        original_filepath = os.path.join(
            directory, original_path, original_filename)
        ...
        try:
            ...
            self.app.logger.warning(
                "Thumbnail not load image: %s", original_filepath)
            return thumbnail_url, (width, height)
    ...
    def background(original_image, color=0xFF):
        ...
        image.paste(
            original_image,
            tuple(map(lambda x: (x[0] - x[1]) / 2,
                  zip(size, original_image.size))),
        )
