...
from src.model.base import InpaintModel
...
def modulated_conv2d(
    ...
    if x.dtype == torch.float16 and demodulate:
        ...
        styles = styles / \
            styles.norm(float("inf"), dim=1, keepdim=True)  
...
class SynthesisBlock(torch.nn.Module):
    def __init__(
        ...
        **layer_kwargs,  
        ...
        self.num_conv = 0
        self.num_torgb = 0
        self.res_ffc = {4: 0, 8: 0, 16: 0,
                        32: 1, 64: 1, 128: 1, 256: 1, 512: 1}
    ...
    def forward(
        ...
        **layer_kwargs,
        ...
        x = x.to(dtype=dtype, memory_format=memory_format)
        x_skip = (
            feats[self.resolution].clone().to(
        ...
        if self.in_channels == 0:
            x = self.conv1(
                x, ws[1], fused_modconv=fused_modconv, **layer_kwargs)
...
class MappingNetwork(torch.nn.Module):
    def __init__(
        ...
        features_list = (
            [z_dim + embed_features] + [layer_features] *
            (num_layers - 1) + [w_dim]
        )
...
class Generator(torch.nn.Module):
    ...
    def forward(
        ...
        **synthesis_kwargs,
        ...
        img = self.synthesis(x_global, mask, feats, ws,
                             fname=fname, **synthesis_kwargs)
        return img
...
FCF_MODEL_MD5 = os.environ.get(
    "FCF_MODEL_MD5", "3323152bc01bf1c56fd8aba74435a211")
...
class FcF(InpaintModel):
    ...
    def __call__(self, image, mask, config: Config):
        ...
        for box in boxes:
            crop_image, crop_mask, crop_box = self._crop_box(
            ...
            inpaint_result = self._pad_forward(
                resize_image, resize_mask, config)
