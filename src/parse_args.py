...
from src.runtime import dump_environment_info
...
def parse_args():
    ...
    parser.add_argument("--cpu-offload", action="store_true",
                        help=CPU_OFFLOAD_HELP)
    parser.add_argument(
        "--disable-nsfw", action="store_true", help=DISABLE_NSFW_HELP)
    parser.add_argument(
        "--sd-cpu-textencoder", action="store_true", help=SD_CPU_TEXTENCODER_HELP
    )
    parser.add_argument("--sd-controlnet",
                        action="store_true", help=SD_CONTROLNET_HELP)
    parser.add_argument(
        "--sd-controlnet-method",
        default=DEFAULT_CONTROLNET_METHOD,
        choices=SD_CONTROLNET_CHOICES,
    )
    parser.add_argument("--sd-local-model-path",
                        default=None, help=SD_LOCAL_MODEL_HELP)
    parser.add_argument(
    ...
    parser.add_argument("--output-dir", type=str,
                        default=None, help=OUTPUT_DIR_HELP)
    parser.add_argument(
    ...
    parser.add_argument("--enable-gfpgan",
                        action="store_true", help=GFPGAN_HELP)
    parser.add_argument(
    ...
    parser.add_argument("--hf_access_token", default="",
                        help=argparse.SUPPRESS)
    parser.add_argument(
        "--sd-disable-nsfw", action="store_true", help=argparse.SUPPRESS
    )
    parser.add_argument(
        "--sd-run-local", action="store_true", help=argparse.SUPPRESS)
    parser.add_argument(
    ...
    if args.load_installer_config:
        if args.installer_config and not os.path.exists(args.installer_config):
            parser.error(
                f"args.installer_config={args.installer_config} not exists")
    ...
    if args.input and args.input is not None:
        ...
        if os.path.isfile(args.input):
            if imghdr.what(args.input) is None:
                parser.error(
                    f"invalid --input: {args.input} is not a valid image file")
        else:
